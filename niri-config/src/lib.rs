...
impl Config {
    ...
    pub fn parse(filename: &str, text: &str) -> Result<Self, ConfigParseError> {
        ...
        // Parse with kdl-rs instead of knuffel
        let mut doc: KdlDocument = dbg!(text.parse())?;
        // Convert the document to a v1 doc if needed, since that's
        // the only thing knuffel can parse.
        doc.ensure_v1();
        // Pass the v1 string to knuffel itself. Parsing per-se should never fail,
        // but knuffel-specific parser failures such as missing expected fields,
        // etc, might still fail.
        Ok(dbg!(knuffel::parse(filename, &doc.to_string()))?)
    }
}
