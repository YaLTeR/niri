...
pub struct ScreenCast {
    ...
    sessions: Arc<Mutex<Vec<InterfaceRef<Session>>>>,
    #[cfg(feature = "xdp-gnome-remote-desktop")]
...
impl ScreenCast {
    async fn create_session(
        ...
        let rd_session_iface = if let Some(rd_session_id) = properties.remote_desktop_session_id {
            ...
            let rd_session_id = rd_session_id.parse().map_err(|err| {
        ...
        match server.at(&path, session).await {
            Ok(true) => {
                ...
                self.sessions.lock().await.push(iface);
            }
...
impl Session {
    ...
    /// Stops the session.
    async fn stop(&self, server: &ObjectServer, ctxt: &SignalEmitter<'_>, reason: StopCastReason) {
    ...
    pub async fn stop_from_stopcast(&self, server: &ObjectServer, ctxt: &SignalEmitter<'_>) {
