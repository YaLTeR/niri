...
impl Session {
    ...
    fn convert_stream_coordinate_space(
        &self,
        _stream_id: &str,
        x: f64,
        y: f64,
    ) -> fdo::Result<Point<f64, Logical>> {
        // FIXME: TODO: convert screencast-local coordinates to global coordinates?

        Ok(Point::new(x, y))
    }

    fn global_extent(&self) -> Size<f64, Logical> {
        Size::new(0., 0.)
    }
}
...
impl Session {
    ...
    async fn notify_pointer_motion_absolute(
        ...
        let pos = self.convert_stream_coordinate_space(stream, x, y)?;
        let global_extent = self.global_extent();

        self.emulate_input(InputEvent::PointerMotionAbsolute {
            event: self.wrap_event(RdPointerMotionAbsoluteEvent(RdAbsolutePosition {
                pos,
                global_extent,
            })),
        });
    ...
    async fn notify_touch_down(
        ...
        self.emulate_input(InputEvent::TouchDown {
            event: self.wrap_event(RdTouchEvent {
                slot,
                extra: RdAbsolutePosition { pos, global_extent },
            }),
        });
        Ok(())
    }
