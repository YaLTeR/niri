...
pub struct RemoteDesktop {
    ...
    // TODO: We can have &mut access, does this need to be arc+mutex?
...
pub struct Session {
    ...
    id_str: String,
    to_niri: calloop::channel::Sender<RemoteDesktopToNiri>,
    // TODO: We can have &mut access, does this need to be arc+atomic?
...
impl Session {
    ...
    pub async fn stop(
        ...
        if let Err(err) = self.to_niri.send(RemoteDesktopToNiri::StopSession {
            session_id: self.id,
        }) {
        ...
        // TODO: remove from [`RemoteDesktop`]'s "sessions" list
    ...
    async fn connect_to_eis(&self, options: ConnectToEisOptions) -> fdo::Result<zvariant::OwnedFd> {
        ...
        if let Err(err) = self.to_niri.send(RemoteDesktopToNiri::NewEisContext {
            ctx,
...
impl RemoteDesktop {
    pub fn new(to_niri: calloop::channel::Sender<RemoteDesktopToNiri>) -> Self {
        Self {
            to_niri,
...
impl Session {
    pub fn new(id: usize, to_niri: calloop::channel::Sender<RemoteDesktopToNiri>) -> Self {
        Self {
            id,
            id_str: id.to_string(),
            to_niri,
...
impl Drop for Session {
    fn drop(&mut self) {
        let _ = self.to_niri.send(RemoteDesktopToNiri::StopSession {
            session_id: self.id,
        });
    }
}
