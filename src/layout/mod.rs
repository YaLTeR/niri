...
impl<W: LayoutElement> Layout<W> {
    ...
    pub fn with_windows(&self, mut f: impl FnMut(&W, Option<&Output>, Option<WorkspaceId>, WindowLayout)) {
        if let Some(InteractiveMoveState::Moving(move_)) = &self.interactive_move {
            f(move_.tile.window(), Some(&move_.output), None, WindowLayout {
                ...
                for mon in monitors {
                    for ws in &mon.workspaces {
                        ...
                        for (tile, window_layout) in ws.tiles_with_window_layouts() {
                            f(tile.window(), Some(&mon.output), Some(ws.id()), window_layout);
                        }
                    }
                }
            }
