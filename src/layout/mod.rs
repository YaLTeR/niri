...
pub struct InteractiveMoveData<W: LayoutElement> {
    ...
    pub(self) initial_pointer_location: Point<f64, Logical>,
    pub(self) pointer_offset: Point<f64, Logical>,
    pub(self) window_offset: Point<f64, Logical>,
}
...
impl<W: LayoutElement> Layout<W> {
    ...
    pub fn with_windows(&self, mut f: impl FnMut(&W, Option<&Output>, Option<WorkspaceId>)) {
        if let Some(move_) = &self.interactive_move {
            f(move_.window.window(), Some(&move_.output), None);
        }
    ...
    pub fn interactive_move_begin(
        ...
        let window = match &mut self.monitor_set {
            MonitorSet::Normal { monitors, .. } => monitors
                ...
                .map(|ws| ws.remove_window_with_col_info(&window, Transaction::new()))
                .next(),
    ...
    pub fn interactive_move_end(&mut self, window: &W::Id) {
        ...
        let position = workspace.get_insert_position(move_.get_pointer_loc());
        match position {
            InsertPosition::NewColumn(column_idx) => workspace.add_window(
                Some(column_idx),
