    ...
    pub fn advance_animations(&mut self) {
        ...
        if let Some((output, pos_within_output, is_scrolling)) = dnd_scroll {
            if let Some(mon) = self.monitor_for_output_mut(&output) {
                ...
                scrolled |= mon.dnd_scroll_gesture_scroll(pos_within_output, 1. / ws_scale);
    ...
    pub fn interactive_move_end(&mut self, window: &W::Id) {
        ...
        match &mut self.monitor_set {
            MonitorSet::Normal {
                ...
                let (mon, insert_ws, position, offset, ws_scale) = if let Some(mon) =
                    monitors.iter_mut().find(|mon| mon.output == move_.output)
                {
                    ...
                    let (position, offset) = match insert_ws {
                        ...
                        InsertWorkspace::NewAt(ws_idx) => {
                            ...
                            // There isn't really a good meaningful offset here. Try to use one
                            // that will give us a better animation.
                            //
                            // FIXME: some jank with ws_idx == 0, and especially with EWAF.
                            let geo_ws_idx =
                                if self.options.empty_workspace_above_first && ws_idx == 0 {
                                    0
                                } else if mon.workspaces.len() - 1 <= ws_idx {
                                    mon.workspaces.len() - 1
                                } else if ws_idx <= mon.active_workspace_idx {
                                    max(ws_idx, 1) - 1
                                } else {
                                    ws_idx
                                };

                            let geo = mon.workspaces_render_geo().nth(geo_ws_idx).unwrap();

                            (position, geo.loc)
                        }
                    };

                    (mon, insert_ws, position, offset, ws_scale)
                } else {
                    let mon = &mut monitors[*active_monitor_idx];
                    let ws_scale = mon.workspace_scale();
                    ...
                    (mon, insert_ws, position, ws_geo.loc, ws_scale)
                };
...
fn compute_workspace_scale(overview_progress: Option<f64>) -> f64 {
    if let Some(p) = overview_progress {
        (1. - p * (1. - OVERVIEW_WORKSPACE_SCALE)).max(0.0001)
    } else {
        1.
    }
}
