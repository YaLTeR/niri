...
fn should_intercept_key(
    ...
    let last_key = if pressed && !is_inhibiting_shortcuts {
        last_key.replace(key_code)
    } else {
        last_key.take()
    };
    ...
    match (final_bind, pressed) {
        ...
        (final_bind, false) if suppressed_keys.contains(&key_code) => {
            ...
            if let Some(bind) = final_bind {
                if bind.release && last_key == Some(key_code) {
                    ShouldInterceptResult::InterceptAndHandle(bind)
                } else {
                    ShouldInterceptResult::InterceptOnly
                }
        ...
        (Some(bind), false) if bind.release && last_key == Some(key_code) => {
            // We don't need to check for shortcut inhibition here because if
            // it was inhibited on press it wouldn't be in last_key.
            ShouldInterceptResult::ForwardAndHandle(bind)
        }
...
/// Finds the specified keybind. Note that release binds can be
/// returned even when pressed=true because we still need to inhibit
/// them.
fn find_bind(
    bindings: &Binds,
    mod_key: ModKey,
    ...
    if let Some(bind) = find_configured_bind(bindings, mod_key, trigger, mods, pressed) {
        ...
        // We prefer the exact match, but also need to return an inexact one.
