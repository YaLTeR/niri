...
//! - Workspace coordinates: X = 1, Y = workspace index. They need to be two-dimensional because 1D
...
pub fn refresh(state: &mut State) {
    ...
    protocol_state.workspace_groups.retain(|output, data| {
        ...
        for group in &data.instances {
            // Send workspace_leave for all workspaces in this group with matching client.
                ...
                for ws in protocol_state.workspaces.values() {
                    if ws.output.as_ref() == Some(output) {
                        for workspace in &ws.instances {
                            if workspace.client().as_ref() == Some(&client) {
...
fn refresh_workspace(
    ...
    match protocol_state.workspaces.entry(ws.id()) {
        Entry::Occupied(entry) => {
            ...
            if check {
                let new_name = ws.name().cloned().unwrap_or_else(|| ws_idx.to_string());
        ...
        Entry::Vacant(entry) => {
            ...
            let mut data = ExtWorkspaceData {
                id: ws.name().cloned(),
                name: ws.name().cloned().unwrap_or_else(|| ws_idx.to_string()),
                coordinates: ArrayVec::from([1, ws_idx as u32]),
                state,
                instances: Vec::new(),
...
impl ExtWorkspaceGroupData {
    ...
    {
        let group = client
            .create_resource::<ExtWorkspaceGroupHandleV1, _, D>(handle, manager.version(), ())
            .unwrap();
...
impl ExtWorkspaceData {
        ...
        D: 'static,
    {
        let workspace = client
            .create_resource::<ExtWorkspaceHandleV1, _, D>(handle, manager.version(), ())
            .unwrap();
    ...
    fn request(
        ...
        match request {
            ext_workspace_handle_v1::Request::Activate => {
                state.activate_workspace(workspace);
            }
            ...
            ext_workspace_handle_v1::Request::Assign { workspace_group } => {
                ...
                {
                    state.assign_workspace(workspace, output);
                }
            }
