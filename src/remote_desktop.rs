...
use crate::input::eis_backend::{
    CombinedEisEventBase, EisEventAdapter, EisInputBackend, PressedCount, ScrollFrame,
};
use crate::input::remote_desktop_backend::{RdEventAdapter, RdKeyboardKeyEvent, RdInputBackend};
...
impl State {
    pub fn on_remote_desktop_msg_from_dbus(&mut self, msg: RemoteDesktopDBusToCalloop) {
        match msg {
            RemoteDesktopDBusToCalloop::RemoveEisHandler { session_id } => {
                if let Some(token) = self.niri.remote_desktop.active_sessions.remove(&session_id) {
                ...
                let token = self
                    ...
                    .insert_source(
                        ...
                        move |event, connection, state| {
                            match Self::handle_eis_request_source_event(
                                event,
                                connection,
                                state,
                                &mut context_state,
            ...
            RemoteDesktopDBusToCalloop::EmulateKeysym {
                ...
                self.process_input_event::<RdInputBackend>(smithay::backend::input::InputEvent::Keyboard { event: RdEventAdapter {
                    session_id,
                    time,
        ...
        Ok(match event {
            Ok(event) => match event {
                EisRequestSourceEvent::Connected => {
                    ...
                    // TODO: filter to negotiated interfaces
                    let seat = connection.add_seat(
                        Some("default"),
                        &MutterXdpDeviceType::to_reis_capabilities(
                            context_state.exposed_device_types,
                        ),
                    );
                ...
                EisRequestSourceEvent::Request(request) => {
                    ...
                    let post_action =
                        Self::handle_eis_request(request, connection, global_state, context_state);
                    connection.flush()?;
                    post_action
                }
                EisRequestSourceEvent::InvalidObject(object_id) => {
                    debug!("EIS invalid object! {:#?}", object_id);
    ...
    fn create_ei_keyboard(
        ...
        (capabilities & 2 << DeviceCapability::Keyboard as u64 != 0 && connection.has_interface("ei_keyboard")).then(|| 
        seat.add_device(
            Some("keyboard"),
            DeviceType::Virtual,
            &[DeviceCapability::Keyboard],
            |device| {
                let keyboard: reis::eis::Keyboard = device.interface().expect("Should exist because it was just defined");

                let file = global_state.niri.seat.get_keyboard().unwrap().with_xkb_state(global_state, |context| {
                    let xkb = context.xkb().lock().unwrap();
    ...
    fn create_ei_mouse(
        ...
        let mut mouse_capabilities = Vec::new();
        ...
        let mut check_mouse_cap = |capability, interface| {
            ...
            // protocol violations.
            if capabilities & 2 << capability as u64 != 0 && connection.has_interface(interface) {
                mouse_capabilities.push(capability);
            }
        };
    ...
    fn create_ei_touchscreen(
        ...
        (capabilities & 2 << DeviceCapability::Touch as u64 != 0
            && connection.has_interface("ei_touchscreen"))
        .then(|| {
            seat.add_device(
                Some("touchscreen"),
                DeviceType::Virtual,
    ...
    fn handle_eis_request(
        ...
        match request {
            ...
            EisRequest::Bind(reis::request::Bind { seat, capabilities }) => {
                if capabilities
                    & MutterXdpDeviceType::to_reis_capability_mask(
                        context_state.exposed_device_types,
                    )
                    != capabilities
                {
                ...
                if let Some(device) =
                    Self::create_ei_keyboard(&seat, capabilities, connection, global_state)
                {
                    device.resumed();
                    context_state.keyboard_device = Some(device);
                }

                if let Some(device) = Self::create_ei_mouse(&seat, capabilities, connection) {
                    device.resumed();
                    context_state.mouse_device = Some(device);
                }

                if let Some(device) = Self::create_ei_touchscreen(&seat, capabilities, connection) {
                    device.resumed();
            ...
            EisRequest::PointerMotion(inner) => {
                global_state.process_input_event(InputEvent::PointerMotion {
            ...
            EisRequest::PointerMotionAbsolute(inner) => {
                global_state.process_input_event(InputEvent::PointerMotionAbsolute {
                    event: EisEventAdapter {
                        session_id: context_state.session_id,
                        inner,
                        extra: (),
            ...
            EisRequest::Button(inner) => {
                global_state.process_input_event(InputEvent::PointerButton {
                    event: EisEventAdapter {
                        session_id: context_state.session_id,
                        inner,
                        extra: (),
            ...
            EisRequest::Frame(inner) => {
                if let Some(scroll_frame) = context_state.scroll_frame.take() {
                    global_state.process_input_event(InputEvent::PointerAxis {
                        event: CombinedEisEventBase {
                            session_id: context_state.session_id,
                            time: inner.time,
                            device: inner.device,
                            data: scroll_frame,
            ...
            EisRequest::KeyboardKey(inner) => {
                ...
                // TODO: DeviceAdded and DeviceRemoved Smithay input events should
                // be sent so the device objects (specifically tablet and touch)
                // get created on Wayland connections. For example, create wl_touch
                // objects when there isn't a physical touch device connected.

                // ^ Maybe they could be sent with StartEmulating and StopEmulating
                // (and paused event)?
