...
To keep a window visible across all workspaces on the current output, use `open-sticky true` or the
`toggle-window-sticky` action; sticky windows are floating windows.
