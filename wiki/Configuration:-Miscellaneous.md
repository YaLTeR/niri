...
This option works the same way as the `spawn` key binding action, so please read about all its subtleties on the [key bindings](./Configuration:-Key-Bindings#spawn) page.
...
Check the [key bindings](./Configuration:-Key-Bindings#custom-hotkey-overlay-titles) wiki page for details.
