...
<sup>Since: 25.05</sup> You can customize the `Mod` key [in the `input` section of the config](./input.md#mod-key-mod-key-nested).
...
The screenshot is both stored to the clipboard and saved to disk, according to the [`screenshot-path` option](./miscellaneous.md#screenshot-path).
