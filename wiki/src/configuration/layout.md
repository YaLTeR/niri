...
> Either way, `default-column-width {}` is most useful for specific windows, in form of a [window rule](./window-rules.md#default-column-width) with the same syntax.
...
>
> If you don't like that, you should uncomment the [`prefer-no-csd` setting](./miscellaneous.md#prefer-no-csd) at the top level of the config.
> Niri will draw focus rings and borders *around* windows that agree to omit their client-side decorations.
>
> Alternatively, you can override this behavior with the [`draw-border-with-background` window rule](./window-rules.md#draw-border-with-background).
...
Shadow drawing will follow the window corner radius set with the [`geometry-corner-radius` window rule](./window-rules.md#geometry-corner-radius).
...
You can also set the color per-output [in the output config](./outputs.md#background-color).
